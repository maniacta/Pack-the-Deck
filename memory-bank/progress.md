...
### 2026-05-13 - 阶段八-端到端可玩验证：完整测试覆盖

#### 新增 BattleSimulator — 无 UI 战斗模拟器 ✅
- [x] `tests/simulation/battle_simulator.gd` — 端到端测试核心类
- [x] 模拟完整战斗流程：抽牌→选牌→出牌→计分→判定
- [x] 支持固定种子确保可复现
- [x] 支持装备效果和 Boss 规则
- [x] 自动选择最优牌型组合（遍历所有 1-5 张组合）
- [x] 弃牌堆洗牌支持

#### 新增 6 个系统单元测试 ✅
| 测试文件 | 覆盖系统 | 用例数 | 关键验证 |
|---------|---------|--------|---------|
| test_equipment_manager.gd | EquipmentManager | 11 | 1×1/2×2/L 形放置、越界/冲突/卸下、相邻检测、通用类别不冲突 |
| test_hand_manager.gd | HandManager | 10 | 添加/移除手牌、切换选中、上限检查、清除、空值拒绝 |
| test_turn_manager.gd | TurnManager | 8 | 回合计数、Boss PLAY_LIMIT/CARD_LIMIT、回合耗尽、信号 |
| test_game_manager.gd | GameManager | 11 | 状态转换、重复防护、游戏结束/通关、完整状态序列 |
| test_shop_manager.gd | ShopManager | 9 | 生成/购买/刷新、金币不足保护、免费刷新追踪、售罄检测 |

#### 新增 2 个端到端模拟测试 ✅
| 测试文件 | 测试层级 | 用例数 |
|---------|---------|--------|
| simulation/test_full_battle.gd | 模拟（无 UI） | 5 |
| simulation/test_edge_cases.gd | 模拟（无 UI） | 9 |

**端到端测试覆盖**:
- [x] 关卡 1 简单通关（4 回合 / 150 分）
- [x] Boss 关卡战斗（花色排除规则）
- [x] 带装备的战斗（完美镜片规则改写）
- [x] 失败场景（不可能达成的目标）
- [x] 多场战斗不同种子稳定性
...
#### 修改文件 ✅
- [x] `scripts/main.gd` — 集成全部 14 个测试文件（从 7 个扩展到 14 个）

#### 新增文件 ✅ (8 个)
- [x] `tests/simulation/battle_simulator.gd` — 战斗模拟器
- [x] `tests/test_equipment_manager.gd`
- [x] `tests/test_hand_manager.gd`
- [x] `tests/test_turn_manager.gd`
- [x] `tests/test_game_manager.gd`
- [x] `tests/test_shop_manager.gd`
- [x] `tests/simulation/test_full_battle.gd`
- [x] `tests/simulation/test_edge_cases.gd`
...
| 测试文件数 | 7 | 14 |
| 测试用例数 | ~50 | ~113+ |
| 覆盖系统数 | 7/14 | 14/14 (100%) |
| 测试层级 | 仅单元 | 单元 + 模拟 + 边界 |

#### 文档更新 ✅
- [x] 更新 architecture.md — 新增测试系统章节、BattleSimulator 文档、测试文件总览表
...
#### 关卡结算面板改进 ✅
- [x] 过关后显示结算面板（得分、奖励、金币）
- [x] 不自动进入下一关，需点击按钮
- [x] 添加"下一关"按钮（有下一关时显示）
- [x] 添加"商店"按钮（预留，暂未开放）
- [x] 添加"重试本关"按钮
- [x] 失败时显示不同按钮组合
- [x] 重置按钮不再自动进入下一关（仅重试当前关卡）
- [x] 预留扩展性（商店按钮、按钮容器布局）

#### 修改文件 ✅
- [x] scenes/battle.tscn - 扩展结果面板结构
- [x] scripts/battle_controller.gd - 新增按钮处理逻辑
...
### 阶段八：测试关卡与可玩验证 ✅ 完成
- [x] 步骤 8.1：测试关卡（3 个关卡已创建并调优）
- [x] 步骤 8.2：端到端游戏流程测试（BattleSimulator + 5 场景）
- [x] 步骤 8.3：边界情况测试（9 个边界场景）
...
### 2026-04-15 - 关卡切换流程设计决策

| 决策项 | 结论 |
|--------|------|
| 过关后行为 | 显示结算面板，不自动进入下一关 |
| 进入下一关方式 | 点击"下一关"按钮 |
| 结算面板内容 | 得分、奖励金币、当前金币 |
| 预留按钮 | 商店按钮（暂未开放） |
| 重置按钮行为 | 仅重试当前关卡，不进入下一关 |
| 失败时按钮 | 隐藏下一关按钮，显示重试按钮 |
| 结果面板尺寸 | 400×220 px |
